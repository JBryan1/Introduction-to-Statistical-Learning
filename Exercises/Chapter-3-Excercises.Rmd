...
### 5. Consider the fitted values that result from performing linear regression without an intercept. In this setting, the ith fitted value takes the form $$\hat{y_i} = x_i\hat{\beta_}$$, where $$\hat{\beta} = \frac{\sum_{j = 1}^{n} x_jy_j}{\sum_{k=1}^{n}x_k^2}$$ Show that we can write $$\hat{y_i} = \sum_{j=1}^na_jy_j$$. What is $a_j$?
